import { CommonModule } from '@angular/common';
import { Component, EventEmitter, Input, OnChanges, Output, SimpleChanges } from '@angular/core';
import { FormBuilder, FormGroup, ReactiveFormsModule, Validators } from '@angular/forms';
import { TranslatePipe } from '../../../../core/translate.pipe';
import { ExpressionInputComponent } from '../expression-input.component';

export interface WebSetTextProperties {
  selector: string;
  value: string;
  waitSelector?: string;
  [key: string]: unknown;
}

/** Property editor for the `Web.Fill` activity (Faz 2 Task 2.6). */
@Component({
  selector: 'app-web-set-text-property',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, TranslatePipe, ExpressionInputComponent],
  templateUrl: './web-set-text-property.component.html',
  styleUrls: ['./web-set-text-property.component.scss'],
})
export class WebSetTextPropertyComponent implements OnChanges {
  @Input() properties: Record<string, unknown> | null | undefined;
  @Output() readonly propertiesChange = new EventEmitter<WebSetTextProperties>();

  readonly form: FormGroup;

  constructor(private readonly fb: FormBuilder) {
    this.form = this.fb.group({
      selector: this.fb.control('', Validators.required),
      text: this.fb.control('', Validators.required),
      waitSelector: this.fb.control(''),
    });
    this.form.valueChanges.subscribe(() => this.emitIfValid());
  }

  ngOnChanges(changes: SimpleChanges): void {
    if (changes['properties']) {
      const props = (this.properties ?? {}) as Partial<WebSetTextProperties> & { text?: string };
      this.form.patchValue(
        {
          selector: props.selector ?? '',
          text: props.value ?? props.text ?? '',
          waitSelector: props.waitSelector ?? '',
        },
        { emitEvent: false },
      );
    }
  }

  get selectorControl() {
    return this.form.get('selector')!;
  }

  get textControl() {
    return this.form.get('text')!;
  }

  private emitIfValid(): void {
    if (this.form.valid) {
      const { selector, text, waitSelector } = this.form.value;
      this.propertiesChange.emit({
        selector,
        value: text,
        ...(waitSelector ? { waitSelector } : {}),
      });
    }
  }
}
